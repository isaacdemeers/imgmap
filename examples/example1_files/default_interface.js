...
function gui_coords_keydown(e) {
  ...
  var obj =
    myimgmap.isMSIE || myimgmap.isOpera
...
function gui_input_change(e) {
  ...
  if (obj.name == "img_href") {
    ...
    if (
      ...
      myimgmap.areas[id].shape != "undefined"
    ) {
      ...
      coords = myimgmap._normCoords(
        coords,
        obj.value,
        "from" + myimgmap.areas[id].shape
      );
      if (props[id]) {
...
function gui_addArea(id) {
  ...
  myimgmap.addEvent(
    props[id].getElementsByTagName("input")[1],
    "keydown",
    gui_cb_keydown
  );
  myimgmap.addEvent(
    props[id].getElementsByTagName("input")[2],
    "keydown",
    gui_coords_keydown
  );
  myimgmap.addEvent(
  ...
  myimgmap.addEvent(
    props[id].getElementsByTagName("input")[3],
    "change",
    gui_input_change
  );
  myimgmap.addEvent(
    props[id].getElementsByTagName("input")[4],
    "change",
    gui_input_change
  );
  myimgmap.addEvent(
    props[id].getElementsByTagName("select")[0],
    "change",
    gui_input_change
  );
  myimgmap.addEvent(
    props[id].getElementsByTagName("select")[1],
    "change",
    gui_input_change
  );
  ...
  if (myimgmap.nextShape) {
    props[id].getElementsByTagName("select")[0].value = myimgmap.nextShape;
  }
...
function gui_htmlBlur() {
  ...
  if (
    oldvalue != elem.value &&
    document.getElementById("dd_output").value == "imagemap"
  ) {
    //dirty
...
function gui_outputChanged() {
  ...
  if (output == "spip") {
    ...
    if (others) {
      if (
        !confirm(
          "CSS maps only support rectangles. Are you sure you want to transform all your areas to rectangles?"
        )
      ) {
        output = outputmode;
        return false;
      ...
      myimgmap.areas.length > 0 &&
      !confirm(
        "CSS maps only support rectangles. Are you sure you want to use this output mode?"
      )
    ) {
      output = outputmode;
      return false;
    ...
    temp +=
      "Please note, that you have to use a positioned container to make use of the absolute coordinates ";
    temp +=
    ...
    temp +=
      "Please note, that you have to attach this code to your image, via the usemap property ";
    temp +=
    ...
    if (myimgmap.areas[i]) {
      if (myimgmap.areas[i].shape && myimgmap.areas[i].shape != "undefined") {
        ...
        html +=
          ...
          '"' +
          ' href="' +
          myimgmap.areas[i].ahref +
          '"' +
          ' target="' +
          myimgmap.areas[i].atarget +
          '" ><em>' +
          myimgmap.areas[i].atitle +
          "</em></a>";
      }
    }
  }
